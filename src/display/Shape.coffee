...
import * as shaders from 'basegl/display/shader/builder'
...
toXArgs = (args) ->
  (toXCode arg for arg in args).join(',')
...
export class Canvas
  ...
  defShape: (fn, args, cfg={}) ->
    ...
    args = args.slice()
    args.unshift "f_#{shape.name}"
    xargs = toXArgs      args
    @addCodeChunk "#{fn}(#{xargs})"
    shape
  ...
  render: (r) ->
    ...
    for arg in @args
      if arg.constructor == Shape
        ...
        args.push arg
    name = @name.toUpperCase() + '_' + (args.length + 1)
    r.canvas.defShape name, args, {keepID: true}
...
export halfPlane = bindShape 'halfPlane'
...
export move    = bindMethod 'move'
export rotate  = bindMethod 'rotate'
export align   = bindMethod 'align'
...
export class GLSLRenderer
  ...
  render: (s) ->
    ...
    defsCode = defsCode + '\n\n\n' + 'shape _main(sdf_sampler2 origin) {\n' + "return f_#{shape.name}(origin);\n}"
