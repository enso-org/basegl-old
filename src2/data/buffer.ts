...
export class Buffer {
    ...
    read(ix:number) { return this.array[ix] }

    write (ix:number, v:number) {
        assert (this.array.length > ix, () => {
            throw `Index ${ix} is too big, array has ${this.array.length} elements`
        })
        this.array[ix] = v
    }
    ...
    resize (newLength:number) {
        ...
        let arrayView = (this.length <= newLength) 
                      ? this.array
...
export class Bindable {
    ...
    get array  () { return this.buffer.array  }
    ...
    constructor (buffer:Buffer) {
        ...
        this.onChangedMultiple = (ixs) => {
            for (let ix of ixs) { this.onChanged(ix) }
        }
