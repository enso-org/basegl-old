...
import {TypedArray, TypedArrayCons} from 'basegl/data/buffer'
...
class NumberBase {
    static bufferType: TypedArrayCons = Float32Array
    _array: TypedArray
    ...
    static from(a: number | TypedArray): void {
        if (typeof a === 'number') {
            const array = new this.bufferType([a])
            new this(array)
        } else {
            new this(a)
        }
